 ...
 * @version $Revision: 40 $
 */
class StompTest extends StompWsTestCase
{
    /**
     * @var Stomp
     */
    ...
    protected function setUp ()
    {
        ...
        $this->Stomp = new StompWs($this->url, $this->origin);
	...
	protected function produce() {
		$producer = new StompWs($this->url, $this->origin);
        $producer->sync = false;
	...
	protected function subscribe() {
		$consumer = new StompWs($this->url, $this->origin);
        $consumer->sync = false;
        $consumer->clientId = 'test';
        $consumer->connect("system", "manager");
		$consumer->subscribe($this->topic, array('activemq.subscriptionName' => 'test'));
		$consumer->unsubscribe($this->topic);
		$consumer->disconnect();
	}

	protected function consume() {
		$consumer2 = new StompWs($this->url, $this->origin);
        $consumer2->sync = false;
        $consumer2->clientId = 'test';
		$consumer2->setReadTimeout(1);
        $consumer2->connect("system", "manager");
        $consumer2->subscribe($this->topic, array('activemq.subscriptionName' => 'test'));
